...
function othercity_change(text){
	var currnet_val = text.options[text.selectedIndex].text;
	if(currnet_val=='Other'){
		//$(".other_city").val("");
		$(".other_city_section").show();
	}else{
		$(".other_city_section").hide();
		$(".other_city").val("");
	}
}
function othercity_change_reg(currnet_val){
	$("#country_id_new").val($("#country_id").val());
	//alert(currnet_val);
	if(currnet_val=='Other'){
		//$(".other_city").val("");
		$(".other_city_section").show();
...
{
	var base_url = $("#base_url").val();
	var action = base_url+ 'common_request/get_list';
	var hash_tocken_id = $("#hash_tocken_id").val();
...
function dropdownChange_new_mob(currnet_id,disp_on,get_list)
{
	var base_url = $("#base_url").val();
	var action = base_url+ 'common_request/get_list';
	var hash_tocken_id = $("#hash_tocken_id").val();
	...
	if(currnet_val !='' && currnet_val != null )
	{
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				if(get_list =='state_list')
				{
					if($(".city_update_mob").length > 0)
					{
						$(".city_update_mob").html('<option value="">Select City</option>');
					}
				}
			}
		});
...
function setDate_day(num_days)
	{
		...
		if(b_date !='')
		{
			var html_days = '<option value="">Day</option>';
		}
		else
		{
	...
	function latest_setDate_day(num_days)
	{
		var b_date = $("#pro_date").val();
		var html_days = '<option value="" selected="">Day</option>';
		if(num_days !='' && num_days > 0)
		{
			for(var ij = 1;ij<=num_days;ij++)
			...
			$("#pro_date").html(html_days);
		}
	}
	function latest_month_year_change()
	{
		var m_date = $("#pro_month").val();
		var y_date = $("#pro_year").val();
		var num_days = '';
		var update_date_ddr = 0;
		if(m_date !='')
		{
...
function reg_generate_otp_verify()
{
	var hash_tocken_id = $("#hash_tocken_id").val();
	...
	var url_req = base_url+'register/generate_otp';
	show_comm_mask();
	var datastring = 'csrf_new_matrimonial='+hash_tocken_id+'&user_id='+user_id+'&country_code='+country_code+'&mobile_number='+mobile_number;
	$.ajax({
		url : url_req,
		type: 'post',
		data: {"csrf_new_matrimonial": hash_tocken_id, "user_id": user_id, "country_code": country_code, 
"mobile_number": mobile_number},
		...
		success: function(data)
		{
			...
			if(data.status == 'success')
			{
				...
				$("#success_message_mv").show();
				$("#resend_link").hide();
				$("#new_mobile_number").html(country_code+'-'+mobile_number);
				...
				$("#displ_mobile_generate").hide();
				stoptimeout();
				setTimeout(function(){ $('#success_message_mv').slideUp(); }, 3000);
			}
			else
			{
				$("#success_message_mv").hide();
				...
				$("#error_message_mv").show();
				stoptimeout();
				setTimeout(function(){ $('#error_message_mv').slideUp(); }, 3000);
			}
			hide_comm_mask();
		}
	});
...
function reg_varify_mobile_check()
{
	var hash_tocken_id = $("#hash_tocken_id").val();
	var base_url = $("#base_url").val();
	var url_req = base_url+'register/varify_mobile_check_otp';
	var otp_mobile = $("#otp_mobile").val();
	var user_id = $("#user_id").val();
	if(otp_mobile =='')
	{
	...
	show_comm_mask();
	$("#error_message_mv").hide();
	var datastring = 'csrf_new_matrimonial='+hash_tocken_id+'&otp_mobile='+otp_mobile+'&user_id='+user_id;
	$.ajax({
		url : url_req,
		type: 'post',
		data: datastring,
		...
		success: function(data)
		{
			...
			if(data.status== 'success')
			{
				...
				$("#success_message_mv").show();
				stoptimeout();
				// setTimeout(function(){ $('#success_message_mv').slideUp();
				// 	//window.location = base_url + 'register/successful';
				// 	//$("#next-to-education-detail").on( "click", function() {
				// 		$("#mobile-varification").modal('hide');
				// 		$('#basic-info-tab').hide();  
				// 		$('#occupation-tab').show();
				// 		$("#abc_1").addClass("bg-clr");
				// 		$(".ni-basic-btn").addClass("bg-clr-f");
				// 	//});
				// }, 3000);
				setTimeout(function(){
					$('#success_message_mv').slideUp();
					$("#mobile-varification").modal('hide');
					//$("#next-to-education-detail").trigger('click');
					//window.location = base_url+'register/successful';
					window.location = base_url+'premium-member';
					// window.location = base_url+'register/step2';
				}, 2000);
			}
			else
			{
...
function update_msg_status(status)
{
	...
	$.ajax({
		...
		success: function(data)
		{
			...
			close_model('myModal_delete');
			if(status=='delete'){
				window.location.href = base_url+'message/inbox/'+mode;
			}
			load_pagination_message();
			tooltip_fun();
			scroll_to_div('main_id_display');
			...
			success: function(data)
			{
				if(data.status == 'success'){
					...
					if(data.success == 'success'){
						$('#bview_contact').hide();
						$('#view_contact').show();
						$('#bview_contact_mob').hide();
						$('#view_contact_mob').show();
					}
					
				}else{
					if(data.login!=''){
						$('#myModal_ViewContactDetails').html(data.login);
					}
					else{
...
function display_childern_status()
{
	...
	{*/
	var marital_status = $("#marital_status option:selected").val();
	if(marital_status !='' && (marital_status !='Never married' && marital_status !='Annulled'))
	{
...
function reg_display_childern_status()
{
	var marital_status = $("#marital_status option:selected").val();
	if(marital_status !='' && (marital_status !='Never married' && marital_status !='Annulled')){
		$("#living_with_or_not").show();
	}
	else{
		if($("#status_children").val()!='undefined' && marital_status !='' && (marital_status !='Never married' && marital_status !='Annulled')){
			$("#living_with_or_not").show();
		}else{
			$("#living_with_or_not").hide();
		}
	}
}
function display_handicapped_details(handicapped)
{
	show_comm_mask();
	if(handicapped == 'Yes'){
		$(".handicapped_details").show();
	}
	else if(handicapped == 'No'){
		$("#handicapped_details").val('');
		$(".handicapped_details").hide();
	}
	hide_comm_mask();
}

//$("#no_of_years_in_artofliving").change(function()
function years_artofliving_reg(no_of_years)
{
	show_comm_mask();
	//var no_of_years = $(this).val();
	var div1 = document.getElementById('show_artoflivingfield');
	if(no_of_years != '0' && no_of_years != ''){
		div1.style.display = 'block';
		$('.teacher_det').show();
	}else{
		div1.style.display = 'none';
		$('.course_completed_field option').removeAttr('selected');
		$('.course_completed_field').val('').trigger('change');
		$('.teaching_courses_field option').removeAttr('selected');
		$('.teaching_courses_field').val('').trigger('change');
		latest_course();
		$('.teacher_name_field').val('');
		$('.teacher_det').hide();
		$('.teacher_no_country_code_field option').removeAttr('selected');
		$('.teacher_no_country_code_field').val('+91').trigger('change');
		$('.teacher_mobile_no_field').val('');
		document.getElementById("Noart_of_living_teacher").checked = true;
		document.getElementById("Yesart_of_living_teacher").checked = false;
		$(".teaching_courses").hide();
		$(".teacher_code").hide();
		$('.teaching_courses_field option').removeAttr('selected');
		$('.teaching_courses_field').val('').trigger('change');
		$('.teacher_code_field').val('');
	}
	hide_comm_mask();
}
function latest_course(){
	$('.pro_date_field option').removeAttr('selected');
	$('.pro_date_field').val('').trigger('change');
	$('.pro_month_field option').removeAttr('selected');
	$('.pro_month_field').val('').trigger('change');
	$('.pro_year_field option').removeAttr('selected');
	$('.pro_year_field').val('').trigger('change');
}
function years_artofliving_web(no_of_years){
	show_comm_mask();
	//var no_of_years = $(this).val();
	var div1 = document.getElementById('show_artoflivingfield');
	if(no_of_years != '0' && no_of_years != ''){
		...
		$('.teacher_no_country_code_field').val('+91').trigger('change');
		$('.teacher_mobile_no_field').val('');
		document.getElementById("art_of_living_teacherno").checked = true;
		document.getElementById("art_of_living_teacheryes").checked = false;
		$(".teaching_courses").hide();
		$(".teacher_code").hide();
		$('.teaching_courses_field option').removeAttr('selected');
		$('.teaching_courses_field').val('').trigger('change');
...
function years_artofliving(no_of_years){
	show_comm_mask();
	var div2 = document.getElementById('show_artoflivingfield_mob');
	if(no_of_years != '0' && no_of_years != ''){
		...
		$('.teacher_no_country_code_field').val('+91').trigger('change');
		$('.teacher_mobile_no_field').val('');
		document.getElementById("art_of_living_teacherno1").checked = true;
		document.getElementById("art_of_living_teacheryes1").checked = false;
		//$('.teacher_det').hide();
		$(".teaching_courses1").hide();
		$(".teacher_code1").hide();
		$('.teaching_courses_field option').removeAttr('selected');
		$('.teaching_courses_field').val('').trigger('change');
		$('.teacher_code_field').val('');
...
$("#Yesart_of_living_teacher").change(function(){
	show_comm_mask();
	var div = document.getElementById('show_art_of_living_teacher');
...
$("#Noart_of_living_teacher").change(function(){
	show_comm_mask();
	var div = document.getElementById('show_art_of_living_teacher');
...
$("#art_of_living_teacheryes").change(function(){
	show_comm_mask();
	$(".teaching_courses").show();
	$(".teacher_code").show();
	hide_comm_mask();
});
$("#art_of_living_teacherno").change(function(){
	show_comm_mask();
	$(".teaching_courses").hide();
	$(".teacher_code").hide();
	$('.teaching_courses_field option').removeAttr('selected');
...
$("#art_of_living_teacheryes1").change(function(){
	show_comm_mask();
	$(".teaching_courses1").show();
	$(".teacher_code1").show();
	hide_comm_mask();
});
$("#art_of_living_teacherno1").change(function(){
	show_comm_mask();
	$(".teaching_courses1").hide();
	$(".teacher_code1").hide();
	$('.teaching_courses option').removeAttr('selected');
	$('.teaching_courses').val('').trigger('change');
	$('.teacher_code').val('');
	hide_comm_mask();
});
...
function accept_horo(id,status)
{
	$('#hrequester_id').val(id);
	$('#hstatus_sent_recieve').val(status);
}
function delete_horo(id,status)
{
	$('#hrequester_id').val(id);
	$('#hstatus_sent_recieve').val(status);
}
function reject_horo(id,status)
{
	$('#hrequester_id').val(id);
	$('#hstatus_sent_recieve').val(status);
}
function reject_horo_request()
{
	var requester_id = $('#hrequester_id').val();
	var status = $('#hstatus_sent_recieve').val();
	var hash_tocken_id = $('#hash_tocken_id').val();
	var base_url = $('#base_url').val();
	var url = base_url+'my_profile/reject_horo_request';
	var sender = 1;
	show_comm_mask();
	$.ajax({
		url: url,
		...
		success: function(data){
			$(".modal-backdrop").hide();
			$("#main_content_ajaxh").html(data.profile_code);
			load_pagination_code_front_end();
			scroll_to_div("main_content_ajaxh");
			update_tocken($("#hash_tocken_id").val());					
			$('#delete_successh').html(data.response);
			$('#delete_successh').slideDown();
			close_model('myModal_delete12h');
			stoptimeout();
			starttimeout('#delete_successh');
			hide_comm_mask();
		}
	});
	return false;
}
function delete_horo_request()
{
	var requester_id = $('#hrequester_id').val();
	var status = $('#hstatus_sent_recieve').val();
	var hash_tocken_id = $('#hash_tocken_id').val();
	var base_url = $('#base_url').val();
	var url = base_url+'my-profile/delete_horo_request';
	var sender = 1;
	show_comm_mask();
	$.ajax({
		url: url,
		...
		success: function(data){ 	
			...
			scroll_to_div("main_content_ajaxh");
			update_tocken($("#hash_tocken_id").val());					
			$('#delete_successh').html(data.errmessage);
			$('#delete_successh').slideDown();
			close_model('myModal_delete12h');
			stoptimeout();
			starttimeout('#delete_successh');
...
function accept_horo_request()
{
	...
	var hash_tocken_id = $('#hash_tocken_id').val();
	var base_url = $('#base_url').val();
	var url = base_url+'my-profile/accept_horo_request';
	show_comm_mask();
	$.ajax({
		url: url,
		type: 'POST',
